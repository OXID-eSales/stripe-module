...
- Remove the automatic sign-in from the "Second Chance" payment link. The mail sent for an unfinished Stripe payment contains `index.php?cl=stripeFinishPayment&id=<orderId>` and nothing else - no token, no secret. Opening it called `Model\Order::stripeReinitializePayment()`, which wrote the order owner's id into the session variable `usr`; that is the variable `User::loadActiveUser()` reads to decide who is signed in, so whoever opened the link was signed in as that customer without ever entering a password. Order history, billing and delivery addresses and placing orders in the customer's name were all reachable from there. The account itself could not be taken over permanently - the shop asks for the current password before the account email or the password can be changed (`InputValidator::checkLogin()`, `AccountPasswordController`) - and guest accounts without a password are the exception where the email could be changed. The link stays valid as long as the order is not paid and not cancelled, which for an abandoned payment is indefinitely, and nothing was logged. `stripeReinitializePayment()` no longer signs anybody in and returns false when no user is signed in; `Controller\StripeFinishPayment::getOrder()` now requires a signed-in user **and** ownership of the order. The previous ownership check only compared ids *if* somebody happened to be signed in, so it covered a logged-in stranger but not the unauthenticated case the second chance mail is actually about. Visitors who are not signed in are redirected to the login page and can open the link from the mail again afterwards.
- Consequence worth knowing before updating: **guest orders can no longer use the second chance link**, because a guest account has no password to sign in with. Those customers have to be pointed to a new order, or the shop has to be on the follow-up described in `SECURITY.md` - a signed, time limited token in the link, which would also give the link an expiry and remove the reliance on the order id being unguessable. The same change was made in the OXID 7 module `osc/stripe-module`.
...
- Rename the `composer.json` key `conflicts` to `conflict`. Composer silently ignores the plural form, so the constraint meant to keep the module out of OXID eShop `<6.10 | ^7.0` was never evaluated and the module could be installed into a shop line it does not support — the incompatibility only surfaced at runtime. The key is renamed, the constraint itself is unchanged. Same fix as in the Amazon Pay module (2.2.1).
