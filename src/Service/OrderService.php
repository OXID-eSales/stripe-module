...
/**
 * Service for managing Stripe order operations
 */
class OrderService
{
    /**
     ...
     * @return bool
     */
    public function stripeIsCancelablePaymentIntent(PaymentIntent $oStripePaymentIntent): bool
    {
     ...
     * @return string
     */
    protected function formatPrice(float $dPrice): string
    {
     ...
     *
     * @param float $dBrutPrice
     * @param float $dVat
     * @return float
     */
    protected function getVatValue(float $dBrutPrice, float $dVat): float
    {
     ...
     * @return array
     */
    public function getBillingAddressParametersFromOrder(CoreOrder $oOrder): array
    {
        $aReturn = [
            'address' => [
        ...
        if (!empty((string) $oOrder->oxorder__oxbillcompany->value)) {
            ...
            $sTranslatedSalutation = $this->language->translateString($oOrder->oxorder__oxbillsal->value);
     ...
     * @return array
     */
    public function getShippingAddressParameters(CoreOrder $oOrder): array
    {
        $aReturn = [
            'address' => [
        ...
        if (!empty((string) $oOrder->oxorder__oxdelcompany->value)) {
            ...
            $sTranslatedSalutation = $this->language->translateString($oOrder->oxorder__oxdelsal->value);
            if (!empty($sTranslatedSalutation)) {
                $aReturn['name'] = $sTranslatedSalutation . ' ';
            }
     ...
     * @return string
     */
    public function getFilledDescriptionText(CoreOrder $oOrder): string
    {
        ...
        $aSubstitutionArray = [
            ...
            '{storeName}' => $this->config->getActiveShop()->oxshops__oxname->value,
     ...
     * @return string
     */
    protected function getCountryCode(string $sCountryId): string
    {
     ...
     * @return string
     */
    protected function getRegionTitle(string $sRegionId): string
    {
     ...
     * @return array
     */
    public function stripeGetOrderFolders(): array
    {
        return $this->config->getConfigParam('aOrderfolder') ?? [];
    }
}
